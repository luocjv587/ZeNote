...
*   **富文本编辑**：
    ...
    *   **图片支持**：直接粘贴或拖拽图片即可插入。
    *   **快捷工具**：右下角悬浮按钮提供“插入当前时间”等功能。
*   **高效管理**：
    *   **笔记本分类**：创建笔记本对笔记进行归类整理，支持按笔记本筛选。
    *   **全局搜索**：侧边栏实时搜索笔记标题和内容。
    *   **页内搜索**：支持 `Ctrl+F` 在当前笔记内高亮查找。
    *   **置顶功能**：重要笔记一键置顶。
    *   **无限滚动**：侧边栏支持分页懒加载，海量笔记也能流畅浏览。
*   **实用工具**：
    *   **数学计算**：选中算式（如 `1+1`）点击计算按钮直接得出结果。
    *   **JSON 格式化**：一键美化 JSON 文本。
    *   **PDF 导出**：将笔记导出为 PDF 文件。
*   **移动端适配**：
    *   **PWA 支持**：可作为应用安装到手机或桌面，离线也能打开（数据需联网）。
    *   **响应式布局**：移动端完美的列表/详情页切换体验。
*   **数据安全**：
...
### 备份
在应用的“设置”页面中，点击“导出数据库”按钮，即可下载包含所有笔记数据的 SQLite 数据库文件（文件名通常为 `zenote-YYYYMMDD-HHmmss.db`）。

### 恢复
如果你需要恢复数据或迁移到新环境：
1. 将备份的 `.db` 文件重命名为 `znote.db`。
2. 将该文件放入项目的 `data/` 目录中，覆盖原有的 `znote.db` 文件。
3. 重启服务即可生效。

```bash
# 示例操作（假设你已经位于项目根目录）
mv /path/to/backup-file.db ./data/znote.db
